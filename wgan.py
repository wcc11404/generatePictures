...
class WGAN(object):
    def __init__(self):
        ...
        self.c=0.01
        self.batch_size=64
        self.learning_rate = 0.002  # 模型默认学习率
        #self.beta1 = 0.5  # 模型默认=。=我也不确定，规范化参数？
        self.epochs = 20000  # 模型默认训练批次
        self.Model_dir = "WGAN"  # 模型参数默认保存位置
    ...
    def buildModel(self):
        ...
        self.d_optim = tf.train.RMSPropOptimizer(self.learning_rate).minimize(self.d_loss,var_list=d_vars)
        self.g_optim = tf.train.RMSPropOptimizer(self.learning_rate).minimize(self.g_loss,var_list=g_vars)

        #权重裁剪器
        clip_ops = []
        for var in d_vars:
            clip_bounds = [-self.c, self.c]
            clip_ops.append(
